...
/**                                                               **/
/** Carlos Guilherme Spinola Cruz  4903387                        **/
/**                                                               **/
/** 26/04/2013                                                    **/
/*******************************************************************/
...
class Cacador {
	...
	*/
	...
	/*
		Caça as palavras desejadas no quadro. O resultado e armazenado em posicoes.
		
		Parametros:
			quadro - Quadro em que a busca sera feita
			palavras - Arranjo das palavras buscadas
	...
	*/
		...
		for(int a =0;a<palavras.length;a++){
			...
			for (int i=0; i<quadro.length;i++) {
				for(int j=0;j<quadro[i].length;j++){
					//Se detectou a primeira letra de uma das palavras, checa a suspeita e em caso positivo, atualiza o array posicoes
					if(palavras[a][0]==quadro[i][j]){
						if(procurarDirecoes(quadro, palavras[a], new int[] { i , j })){
							posicoes[a][0]=i;
							posicoes[a][1]=j;
						}
					}
	...
	static boolean procurarDirecoes(char[][] quadro, char[] palavra, int[] posicao){

		int acertos_direita = 0, acertos_esquerda = 0, acertos_cima = 0, acertos_baixo = 0;

		//Loop por cada letra da palavra, checando simultaneamente cada direcao possivel que a palavra pode seguir (direita, esquerda, cima, baixo)
		for(int a=0;a<palavra.length;a++){
			//BAIXO:
			if((quadro.length - posicao[0]) > palavra.length){
				if(quadro[posicao[0]+a][posicao[1]] == palavra[a]){acertos_baixo++;}
			}
			//CIMA:
			if(posicao[0] > palavra.length){
				if(quadro[posicao[0]-a][posicao[1]] == palavra[a]){acertos_cima++;}
			}
			//ESQUERDA:
			if(posicao[1] > palavra.length){
				if(quadro[posicao[0]][posicao[1]-a] == palavra[a]){acertos_esquerda++;}
			}
			//DIREITA:
			if((quadro[posicao[0]].length - posicao[1]) > palavra.length){
				if(quadro[posicao[0]][posicao[1]+a] == palavra[a]){acertos_direita++;}
			}
		}

		if(acertos_direita==palavra.length || acertos_esquerda==palavra.length || acertos_cima==palavra.length || acertos_baixo==palavra.length){
			return true;
		}
		return false;
	}
	...
	*/
	public static void main (String[] args) {
		...
		imprimeQuadro (quadro3);
		cacaPalavras (quadro3, palavras3, posicoes3);
		imprimeResposta (palavras3, posicoes3);

		imprimeQuadro (quadro4);
		cacaPalavras (quadro4, palavras4, posicoes4);
		imprimeResposta (palavras4, posicoes4);
